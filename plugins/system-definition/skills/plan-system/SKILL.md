...
description: >
  ...
  technology choices, C4 diagrams, or solution layout (use /architecture:design-architecture);
...
order*. Produces a `PLAN.md` that `/architecture:design-architecture` turns into concrete architectural
decisions and that `/development:build-system` executes against.
...
matters; the interesting decisions get deferred to `/architecture:design-architecture`.
...
- **Choosing concrete technologies, drawing C4 diagrams, or laying out projects** → use `/architecture:design-architecture`.
...
`/architecture:design-architecture` — don't write them here.
...
3. **Draft the data model.** List entities, key relationships, where multi-tenancy fits.
   Don't write schemas yet — that's `/architecture:design-architecture`'s job. Do mark which fields are PII.
...
7. **Surface open questions** that `/architecture:design-architecture` needs answered — usually scale
...
- **Pin the build order, not the tech.** This phase decides *what gets built and in what
  order*. Specific provider/library choices are `/architecture:design-architecture`'s job; don't pre-empt them.
...
- **No "TBD" without a date.** If a choice is genuinely deferred, say so explicitly as an
  open question for `/architecture:design-architecture`, not as a silent gap.
...
- One epic at a time in `/development:build-system` — never start epic N+1 before epic N's tests pass.
- Modules map to .NET projects, not to UI screens. UI organization is `/architecture:design-architecture`'s concern.
- The plan must not contradict the guardrails. If it must deviate, surface the conflict and
  record it as an ADR in `DECISIONS.md` (written during `/architecture:design-architecture`) — never deviate silently.
...
- **Writing schemas in the data-model sketch.** Stay conceptual — entities and relationships,
  not column types or migrations. Schemas are `/architecture:design-architecture`'s output.
...
`PLAN.md` is a local document; the workflow manages work in GitHub. Once the plan passes validation, **publish it to the backlog** with [`sync-backlog`](../sync-backlog/SKILL.md): epics become `type:epic` issues, the capabilities each epic delivers become `type:feature` sub-issues, and every feature lands on the project's Projects v2 board in **Backlog**. Stage 2 then architects from those issues. Don't hand `PLAN.md` straight to architecture — publish first so the backlog is the single source of truth.

## Related skills

- [`synthesize-spec`](../synthesize-spec/SKILL.md) — produces the `SPEC.md` this skill consumes (the previous phase).
- [`sync-backlog`](../sync-backlog/SKILL.md) — publishes this `PLAN.md` into GitHub issues + the backlog board (the next step, closing Stage 1).
- `/architecture:design-architecture` — reads the published feature issues and produces `ARCH.md` plus C4 diagrams (Stage 2).
- `/development:build-system` — implements features off the board, one issue at a time.
