...
description: >
  ...
  (use /architecture:design-architecture); breaking work into epics (use ../plan-system/SKILL.md).
license: MIT
...
6. List what is explicitly out of scope for v1. This is load-bearing — the workflow will refuse to
   build it in `/development:build-system`.
...
- No technology choices in this document — those belong in
  `/architecture:design-architecture`. Talk about *what*, not *how*.
...
- `/architecture:design-architecture` — later reads *Personas* (to derive
