...
description: >
  ...
  DO NOT USE FOR: deciding what the system does (use /system-definition:synthesize-spec); breaking work
  into epics and build order (use /system-definition:plan-system); generating source code (use
...
Stage 2 reads from the **GitHub backlog**: the `type:feature` issues `/system-definition:sync-backlog`
published. The architecture must cover every open feature; as you architect each one you move its
card **Backlog → Ready** and leave a short architecture note as an issue comment, so Stage 3 can pull
"Ready" features off the board with the design context attached. `PLAN.md`/`SPEC.md` remain the
narrative source, but the issues are the worklist and the system-of-record for what's been architected.
...
from `/development:dotnet-aspire-base`. For the architecture-style decision guide
...
- **Deciding what the system does** → use `/system-definition:synthesize-spec`.
- **Breaking work into epics / build order** → use `/system-definition:plan-system`.
- **Generating source code** → use `/development:build-system`.

## Inputs

| Input | Required | Description |
|---|---|---|
| GitHub feature backlog | Yes | The `type:feature` issues on the project board (from `/system-definition:sync-backlog`). The worklist the architecture must cover; each gets moved Backlog → Ready as it's designed. |
| `PLAN.md` | Yes | Produced by `/system-definition:plan-system`. Epics, modules, RBAC, integration surface, background work — the narrative behind the issues. |
| `SPEC.md` | Always consulted | Capabilities, personas, success metrics — the *why* behind the plan. |
| `workflow.json` | When present | Cloud target, declared connectors, capabilities, skill marketplaces, and the `github` block (`repo`, `project`) used to read/move the backlog. |
...
1. **Pull the backlog.** Read `workflow.json.github`, then list the open `type:feature` issues on
   the board (`gh issue list -R OWNER/NAME --label type:feature --state open --json number,title,body,url`).
   This is the worklist: the architecture must have a home for every one of these features. If the
   backlog is empty, send the user to `/system-definition:sync-backlog` first.
2. **Re-read the guardrails.** Every choice must comply. If something in the plan needs to
   deviate, that deviation is an ADR in `DECISIONS.md`.
3. **Draw the C4 L1 context diagram first** — actors and the system as one box. Forces clarity
   on integrations.
4. **Draw the C4 L2 container diagram.** Use Aspire's resource model as the starting point: the
...
5. **Name the projects.** One `Application.<BoundedContext>` per epic from the plan, following
   the naming conventions in the guardrails.
6. **Pin cross-cutting wiring.** Choose a single provider/library for each concern (OTel, MAF,
   RBAC, multi-tenancy, caching, background jobs, idempotency). Ambiguity here causes weeks of
   churn at build time. Reuse the stack skills (e.g. `/development:dotnet-aspire-base`
   for the backbone, `/development:agent-framework-csharp` for MAF) — they encode the *how*.
7. **Decide cloud topology.** Regional layout, networking, managed identity, secret store, and
   observability sinks. Cloud-specific code stays behind `Infrastructure.<Cloud>` interfaces so
   a second cloud target is finite.
8. **Concretize the data model.** Turn the plan's entity sketch into EF Core entities with
   multi-tenancy applied. Mark PII fields with `[Pii]`.
9. **Concretize the API surface.** Minimal-APIs grouped by bounded context, all versioned
   (`/api/v1/...`), all returning Problem Details on error, idempotency keys on writes.
10. **Design the MAF agents.** At minimum the industry chatbot. Each agent's tools include the
   channels from installed connectors plus domain-specific actions. Persist conversations.
11. **Design the SPA.** Dashboard shell, chatbot slide-over panel, per-bounded-context feature folders.
12. **Write the ADRs** for every non-default decision, then refine and run the adversarial
    review under [Validation](#validation) before declaring the phase complete.
13. **Mark architected features Ready.** Once `ARCH.md` covers a feature, move its card
    **Backlog → Ready** on the board and add an architecture note as an issue comment — the
    target context(s)/module, the ADRs that apply, and the acceptance shape — so Stage 3 inherits
    the design. Confirm every open `type:feature` issue is either Ready or has a recorded reason it
    isn't (e.g. deferred to a later epic). See the gh commands below.

## Backlog board (gh commands, inlined)

Reading the worklist and moving cards is part of this phase. The board is the `github.project`
from `workflow.json`; discover field/option ids once and reuse.

```bash
OWNER=abrahamFerga; REPO=$OWNER/<name>; PROJECT=<n>
PID=$(gh project view "$PROJECT" --owner @me --format json --jq '.id')
gh project field-list "$PROJECT" --owner @me --format json   # -> Pipeline field id + Ready option id

# the worklist
gh issue list -R "$REPO" --label type:feature --state open --json number,title,body,url

# architecture note on the feature
gh issue comment <feature#> -R "$REPO" --body $'**Architecture (Stage 2)**\nContext/module: ...\nADRs: ADR-00NN\nAcceptance: ...'

# move its card Backlog -> Ready (find the item id by matching the issue url)
ITEM=$(gh project item-list "$PROJECT" --owner @me --format json --jq '.items[] | select(.content.url=="<issue-url>") | .id')
gh project item-edit --id "$ITEM" --project-id "$PID" --field-id <pipeline-fid> --single-select-option-id <ready-oid>
```

This skill **writes to issues/board only** (comments + the Pipeline field). It never edits code and
never closes issues — that's Stage 3.
...
- `/system-definition:plan-system` — produces the `PLAN.md` this skill consumes (the narrative behind the issues).
- `/system-definition:sync-backlog` — publishes the `type:feature` issues this skill reads and moves to Ready (the upstream that fills the board).
- `/development:work-next-issue` — pulls the Ready features this skill produced off the board and implements them (the next phase).
- `/development:build-system` — reads `ARCH.md` (and the supporting docs) and generates the code for a feature.
...
- `/development:dotnet-aspire-base` — the stack skill that encodes the *how* for the concrete .NET 10 + Aspire solution backbone.
- `/development:pluggable-connectors` — the pattern for the connector registry and `IChannel` / `IIntegration` contracts referenced in the container diagram.
