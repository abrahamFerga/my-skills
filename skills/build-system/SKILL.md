...
name: build-system
description: >
  Generate the working repo for a system from ARCH.md plus the stack and pattern skills —
  solution, projects, code, tests, IaC, and frontend, all consistent with the enterprise
  guardrails. Runs one epic at a time in PLAN.md order, guardrail-checked at every checkpoint
  via build, test, and validate-system, with the Foundations epic always first. Phase 5 of the
  workflow: the synthesis phase that turns the architecture into a buildable system; composes
  stack and pattern skills, invents no new technology choices.
  ...
  DO NOT USE FOR: planning the work or build order (use ../plan-system/SKILL.md); making architecture
  or technology decisions (use ../design-architecture/SKILL.md); validating an already-built system
  (use ../validate-system/SKILL.md); installing the runtime verification surface (use
...
Phase 5 — the synthesis phase. Reads `ARCH.md` and produces a working repo: solution +
projects + code + tests + IaC + frontend, all consistent with the enterprise guardrails.
Composes the stack and pattern skills; does not invent new technology choices.
...
- `ARCH.md` exists, `DECISIONS.md` is in place, and [`validate-system`](../validate-system/SKILL.md) passed cleanly.
...
- **Planning the work / build order** → use [`plan-system`](../plan-system/SKILL.md).
- **Making architecture or technology decisions** → use [`design-architecture`](../design-architecture/SKILL.md).
- **Validating an already-built system** → use [`validate-system`](../validate-system/SKILL.md).
...
|---|---|---|
| `ARCH.md` | Yes | Produced by [`design-architecture`](../design-architecture/SKILL.md). The solution layout and cross-cutting wiring this skill executes. |
| `PLAN.md` | Yes | Walked sequentially — one epic at a time, never starting N+1 before N is green. |
...
Generation runs **one epic at a time** (per `PLAN.md` order), and after each epic the system
must pass [`validate-system`](../validate-system/SKILL.md) plus `dotnet build` + `dotnet test`.
The Foundations epic is always first.

### Epic 1: Foundations (always)
...
2. Invoke [`dotnet-architecture`](../dotnet-architecture/SKILL.md) to lay out the domain /
...
11. Install the runtime verification surface via [`verify-runtime`](../verify-runtime/SKILL.md)
    ...
    when there's a SPA).
12. Run `dotnet build` and `dotnet test`. Run [`validate-system`](../validate-system/SKILL.md).
    Fix any failures before moving on.

### Epic 2..N: Per the plan

For each subsequent epic:

1. Re-read `PLAN.md` for the epic's capabilities + modules.
2. For each module, choose the right stack/pattern skills (e.g. capability skills like `ocr`,
   `vector-db` if relevant).
3. Generate domain entities + application handlers + API endpoints + EF migrations + tests + UI screens.
4. Wire the epic into the dashboard navigation.
5. Run `dotnet build` + `dotnet test` + [`validate-system`](../validate-system/SKILL.md). Do not
   move on if any check fails.
...
4. Final [`validate-system`](../validate-system/SKILL.md). Final `dotnet build --configuration
...
- **Compose, don't reinvent.** Stack skills that exist —
  ...
  [`dotnet-architecture`](../dotnet-architecture/SKILL.md) — encode the *how* per technology, and
...
declared done. A system that lacks any of these is not "enterprise-ready" and [`validate-system`](../validate-system/SKILL.md)
...
- All epics from `PLAN.md` are generated and green.
- [`validate-system`](../validate-system/SKILL.md) exits 0.
...
This is the terminal skill of the workflow loop. The next phase is human review and iteration —
typically circling back to update `SPEC.md`, then re-running
[`plan-system`](../plan-system/SKILL.md) / [`design-architecture`](../design-architecture/SKILL.md) /
build-system for the changed parts.

- [`design-architecture`](../design-architecture/SKILL.md) — produces the `ARCH.md` this skill consumes (the previous phase).
- [`validate-system`](../validate-system/SKILL.md) — the guardrail check run at every epic checkpoint and the final pass.
...
- [`dotnet-architecture`](../dotnet-architecture/SKILL.md) — the stack skill that lays out the domain / application / infrastructure layering this skill fills in.
