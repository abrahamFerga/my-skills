...
- **Deep Aspire app-model work** (declaring resources, service discovery, the
  dashboard, `AddProject`/`AddContainer`) → use [`../aspire`](../aspire/SKILL.md).
...
- **EF Core data modeling** (entities, configurations, migrations, query tuning;
  ...
  [`../entity-framework-core`](../entity-framework-core/SKILL.md).
...
| Aspire | **13.x** | Local orchestration; AppHost + ServiceDefaults — see [`../aspire`](../aspire/SKILL.md) |
| EF Core | **10** | Persistence in Infrastructure via the **Npgsql** provider (`Npgsql.EntityFrameworkCore.PostgreSQL` 10.0.x) — see [`../entity-framework-core`](../entity-framework-core/SKILL.md) |
...
owned by [`../dotnet-aspire-base`](../dotnet-aspire-base/SKILL.md). **Defer to it for
...
| Treating AppHost as the deployable | AppHost is local-only orchestration; the `Api` host is what deploys. See [`../aspire`](../aspire/SKILL.md). |
...
- [references/solution-structure.md](references/solution-structure.md) — The layer
  ...
  generation is deferred to [`../dotnet-aspire-base`](../dotnet-aspire-base/SKILL.md).
